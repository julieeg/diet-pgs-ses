...
# ==============================================
## Apply z-score standardization to PGS 
# ==============================================

analysis <- analysis %>% mutate(across(pgs, ~zscore(.)))
...
# ================================================
## mbase: lm of macro PGS with FFQ traits
# ================================================
diet_vars_cont <- diet_vars[!endsWith(names(diet_vars),".lab")]
lm_pgs_ffq <- do.call(rbind.data.frame, lapply(1:length(pgs_labs), function(p) {
  do.call(rbind.data.frame, lapply(1:length(diet_vars_cont), function(v) {
    print_lm(exposure=names(pgs_labs)[p], outcome=names(diet_vars_cont)[[v]], covariates = m_base, data=analysis, 
             label=diet_vars_cont[[v]]) 
  }))
})) ; lm_pgs_ffq

lm_pgs_ffq %>% fwrite("../data/results/lm_prefpgs_ffqCont_mBase_testing.csv")


# ================================================
## mbase: lm of macro PGS with 24HR traits
# ================================================
lm_pgs_24hr <- do.call(rbind.data.frame, lapply(1:length(pgs_labs), function(p) {
  do.call(rbind.data.frame, lapply(1:length(diet24hr_vars), function(v) {
    print_lm(exposure=names(pgs_labs)[p], outcome=names(diet24hr_vars)[[v]], covariates = m_base, data=analysis, 
             label=diet24hr_vars[[v]]) 
  }))
})) ; lm_pgs_24hr

lm_pgs_24hr %>% fwrite("../data/results/lm_prefpgs_24hr_mBase_testing.csv")
