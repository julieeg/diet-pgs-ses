...
educ_4lvl_labs = list( "College graduate" = 1, "College gradute" = 5, "Some college" = 6,
...
educ_id <- fread("/humgen/florezlab/UKBB_app27892/UKBB_app27892_download_may_2023/ukb672670.tab.gz",
  ...
  mutate(educ_level.lab = descr_label.fun(., "educ_level", educ_level_labs),
         ...
         educ_4lvl.lab = descr_label.fun(., "educ_level", educ_4lvl_labs, ordered = T))
...
prefs_id <- fread("/humgen/florezlab/UKBB_app27892/UKBB_app27892_download_jul_2024/ukbb_app27892_diet_preference_11062024.csv") %>%
  ...
  select(id, pref_sweet_food= p20732, 
         ...
         pref_bitter_food = p20716)


## Macronutrient PGS -------------------------------

pgs_id <- left_join(fread("../data/processed/pgs/ukb_allchr_Y1_MA_pgs") %>% 
                      rename(carb_pgs=score_sum), 
                    fread("../data/processed/pgs/ukb_allchr_Y2_MA_pgs") %>% 
                      rename(fat_pgs=score_sum), by="id") %>%
  left_join(fread("../data/processed/pgs/ukb_allchr_Y3_MA_pgs") %>% 
              rename(prot_pgs=score_sum), by="id")


## Macronutrient phenotypes ------------------------------

macros_id <- fread("../data/processed/gwas/ukb_phenos_gwas_macros_MA.txt") %>%
  rename(id=IID) %>% select(-FID)


phenos <- left_join(macros_id, base_id, by="id") %>% 
...
covars <- fread("../data/processed/ukb_phenos_gwas_MA.txt") %>% mutate(
  sex.lab = case_when(sex==0~"Female", sex==1~"Male"),
  health.lab = descr_label.fun(., "health", health_labs, ordered=T),
  #health.lab = as.factor(case_when(
   # health==1~"Poor", health==2~"Fair",
  #  health==3~"Good", health==4~"Excellent",
  #  TRUE~as.character(NA)), levels=c("Poor", "Fair", "Good", "Excellent")),
  smoking.lab = as.factor(case_when(
    smoke_former==1~"Previous",
    smoke_current==1~"Current",
    smoke_former==0 & smoke_current==0 ~ "Never",
    TRUE~as.character(NA)), levels=c("Never", "Former", "Current")),
  alcohol.lab = descr_label.fun(., "alcohol", alcohol_labs, ordered=T)) %>%
  #alcohol.lab = case_when(
  #  alcohol==0~"Never", alcohol==1~"Special occasions only",
  #  alcohol==2~"One to three times a month", 
  #  alcohol==3~"Once or twice a week",
  #  alcohol==4~"Three or four times a week",
  #  alcohol==5~"Daily or almost daily",
  #  TRUE~as.character(NA))) %>%
  select(-c("carb_pheno", "fat_pheno", "prot_pheno", -"ac"))

phenos <- left_join(phenos, covars, by="id")
...
phenos %>% fwrite("../data/processed/ukb_allphenos_analysis_MA.csv")
phenos %>% saveRDS("../data/processed/ukb_allphenos_analysis_MA.rda")
...
analysis <- phenos %>% filter(t2d_case==0) %>%
  
  ## Calculate quantiles of PGS ----------------------
  filter(complete.cases(carb_pgs, fat_pgs, prot_pgs)) %>%
...
analysis %>% fwrite("../data/processed/ukb_analysis_MA.csv")
analysis %>% saveRDS("../data/processed/ukb_analysis_MA.rda")
